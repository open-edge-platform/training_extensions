...
class TestDEIMDFine:
    ...
    def test_loss_computation(self, model_name: str, label_info: int, fxt_data_module) -> None:
        ...
        expected_losses = ["loss_vfl", "loss_bbox", "loss_giou", "loss_fgl", "loss_ddf", "loss_mal"]
        
        # Check that at least some expected losses are present
        loss_keys = [key for key in output.keys() if key.startswith("loss_")]
        assert len(loss_keys) > 0
        
        # Check for main losses (may have auxiliary losses too)
        for loss_name in expected_losses:
            main_loss_present = any(loss_name in key for key in loss_keys)
            if main_loss_present:
                # If main loss is present, check it's a tensor
                main_loss_key = next(key for key in loss_keys if key == loss_name)
                assert isinstance(output[main_loss_key], torch.Tensor)
    ...
    def test_multi_scale_training(self) -> None:
        ...
        # Multi-scale should be passed to the underlying model
        created_model = model._create_model()
        assert created_model.multi_scale is True
    ...
    def test_dummy_input_generation(self) -> None:
        ...
        for batch_size in [1, 2, 4]:
            ...
            assert dummy_input.batch_size == batch_size
            assert dummy_input.images.shape == (batch_size, 3, 640, 640)
