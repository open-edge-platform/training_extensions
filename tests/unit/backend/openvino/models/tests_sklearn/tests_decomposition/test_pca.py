...
"""Unit test for PCA model wrapper with OpenVINO optimization for OTX."""
...
from otx.backend.openvino.models.sklearn import PCA
...
class TestPCA(unittest.TestCase):
    def test_pca(self):
        ...
        model = PCA(**params, use_openvino=True)
        start = timer()
        model.fit(x_train)
