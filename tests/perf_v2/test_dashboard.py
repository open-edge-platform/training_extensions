...
from plot_benchmark import BenchmarkDashboard
...
def test_dashboard_basic():
    ...
    if dashboard.benchmark_data:
        ...
        try:
            ...
            first_version = list(dashboard.benchmark_data.keys())[0]
            first_task_name = list(dashboard.benchmark_data[first_version].keys())[0]
            first_task = OTXTaskType(first_task_name)
            first_dataset = list(dashboard.benchmark_data[first_version][first_task_name].keys())[0]
            
            # Test accuracy comparison plots
            plots = dashboard.create_accuracy_comparison_plots(first_task, first_dataset)
            print(f"✓ Created accuracy comparison plots for {first_task.value} - {first_dataset}")
            
            # Test latency comparison plots
            plots = dashboard.create_latency_comparison_plots(first_task, first_dataset)
            print(f"✓ Created latency comparison plots for {first_task.value} - {first_dataset}")
            
            # Test training time comparison
            plot = dashboard.create_training_time_comparison_plot(first_task, first_dataset)
            print(f"✓ Created training time comparison plot for {first_task.value} - {first_dataset}")
            
            # Test epoch comparison
            plot = dashboard.create_epoch_comparison_plot(first_task, first_dataset)
            print(f"✓ Created epoch comparison plot for {first_task.value} - {first_dataset}")
            
            # Test iteration time comparison
            plot = dashboard.create_iter_time_comparison_plot(first_task, first_dataset)
            print(f"✓ Created iteration time comparison plot for {first_task.value} - {first_dataset}")
            
            # Test scatter plot
            fig = dashboard.create_latency_accuracy_scatter(first_task, first_dataset)
...
def test_dashboard_run():
    ...
    try:
        # This would normally run indefinitely, but we'll just test the setup
        dashboard.app.run_server(debug=True, port=8050, host='0.0.0.0')
    except Exception as e:
