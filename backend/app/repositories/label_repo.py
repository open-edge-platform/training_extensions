...
class LabelRepository(BaseRepository[LabelDB]):
    ...
    def list_by_project_id(self, project_id: str) -> list[LabelDB]:
        """Get labels by project ID."""
        return self.db.query(LabelDB).filter(LabelDB.project_id == project_id).all()

    def delete_batch_in_project(self, obj_ids: list[str], project_id: str) -> None:
