...
class TestPipelineServiceIntegration:
    ...
    def test_get_pipeline_metrics_success(self, fxt_pipeline_service, fxt_db_models, db_session):
    ...
    def test_get_pipeline_metrics_no_data(self, fxt_pipeline_service, fxt_db_models, db_session):
    ...
    def test_get_pipeline_metrics_no_model(self, fxt_pipeline_service, db_session):
        """Test retrieving metrics for pipeline without model raises error."""
        pipeline_id = uuid4()
        pipeline_db = PipelineDB(
            id=str(pipeline_id),
            name="Test Pipeline",
        ...
        with pytest.raises(ValueError, match="Cannot get metrics for a pipeline without an associated model"):
            fxt_pipeline_service.get_pipeline_metrics(pipeline_id)
