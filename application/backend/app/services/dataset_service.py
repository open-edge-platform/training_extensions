...
class DatasetService(BaseSessionManagedService):
    ...
    def list_dataset_revision_items(
        ...
        """
        parquet_path = self._get_revision_parquet_path(project_id, revision_id)
        ...
        try:
            ...
            if "revision_" in str(parquet_path.parent.name) and parquet_path.parent.parent.name != "dataset_revisions":
    ...
    def get_dataset_revision_item(
        ...
        try:
            # Read the dataframe and filter by ID
            df = pl.read_parquet(parquet_path)
            ...
            if "id" in df.columns:
                ...
                # Fallback: try to use the index or image column
