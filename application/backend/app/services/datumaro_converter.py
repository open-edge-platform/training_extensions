...
class ClassificationSample(Sample):
    ...
    label: int = label_field(dtype=pl.UInt32(), is_list=False)
...
class MultilabelClassificationSample(Sample):
    ...
    image: str = image_path_field()
    image_info: ImageInfo = image_info_field()
    label: NDArrayInt = label_field(dtype=pl.UInt32(), multi_label=True)
...
class DetectionSample(Sample):
    ...
    label: NDArrayInt = label_field(dtype=pl.UInt32(), is_list=True)
    confidence: NDArrayFloat32 | None = numeric_field(dtype=pl.Float32(), is_list=True)
    subset: Subset = subset_field()
