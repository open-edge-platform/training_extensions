...
from app.models.media import MediaListPredictionRequest, MediaPredictionRequest, VideoRange
...
def _test_convert_sequential(
    ...
    while perf_counter() < time_point_to_finish:
        ...
        loaded_media = LoadedMedia(
            single_media=[],
            video_frames={video: [Frame(frame_index=_, skip=False) for _ in range(start_index, end_index)]},
        )
