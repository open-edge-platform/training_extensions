...
def __init(
    ...
    for frame_index in range(batch_size):
        frame = video_service.extract_video_frame(video_path, frame_index)
...
def _test_inference(
    ...
    while perf_counter() < time_point_to_finish:
        ...
        latency = (perf_counter() - batch_start) * 1e3 / len(result.keys())
        latencies += [latency for _ in range(len(result.keys()))]
    return latencies
