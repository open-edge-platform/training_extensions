...
const getDisplayedPercentages = (): number[] => {
    ...
    return texts.map((el) => Number(el.textContent?.match(/(\d+)%/)?.[1]));
};
...
describe('ThreeSectionRange', () => {
    it('labels each percentage with the corresponding subset name', () => {
        ...
        expect(screen.getByText('Training 70%')).toBeInTheDocument();
        expect(screen.getByText('Validation 20%')).toBeInTheDocument();
    ...
    it('renders a zero percentage for every subset when there are no items', () => {
        ...
        expect(screen.getByText('Training 0%')).toBeInTheDocument();
