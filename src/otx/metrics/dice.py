...
NOTE: Dice metric class is a direct copy from `torchmetrics.classification.Dice` in torchmetrics v1.6.0.
In torchmetrics v1.7.0, the Dice metric was removed from the `classification` subpackage and replaced with
`DiceScore` in the `segmentation` subpackage.

Unlike the original `Dice` metric, `DiceScore` computes the Dice coefficient per class and then averages
across classes. This differs from the original implementation, which computes the Dice score globally
across all samples and classes.

This is a temporary workaround to maintain consistent behavior for the semantic segmentation task in OTX.
TODO: Remove this once OTX is updated to support `DiceScore` from the `segmentation` subpackage.
"""

from __future__ import annotations

from typing import TYPE_CHECKING, Any, Callable, Literal, no_type_check
...
class Dice(Metric):
    ...
    def __init__(
        ...
        threshold: float = 0.5,
        average: Literal["micro", "macro", "none"] | None = "micro",
        mdmc_average: str | None = "global",
        ...
        **kwargs: Any,  # noqa: ANN401
        ...
        if average not in allowed_average:
            msg = f"The `average` has to be one of {allowed_average}, got {average}."
        ...
        if average not in ("micro", "macro", "samples"):
            msg = f"The `reduce` {average} is not valid."
            raise ValueError(msg)

        if mdmc_average not in (None, "samplewise", "global"):
            msg = f"The `mdmc_reduce` {mdmc_average} is not valid."
            raise ValueError(msg)

        if average == "macro" and (not num_classes or num_classes < 1):
            msg = "When you set `average` as 'macro', you have to provide the number of classes."
            raise ValueError(msg)

        if num_classes and ignore_index is not None and (not ignore_index < num_classes or num_classes == 1):
            msg = f"The `ignore_index` {ignore_index} is not valid for inputs with {num_classes} classes"
            raise ValueError(msg)

        default: Callable = list
        reduce_fn: str | None = "cat"
