...
from otx.core.types import AnomalyLabelInfo, CollateMode, ImageColorChannel, OTXTaskType
...
class AnomalyDataset(OTXDataset):
    ...
    def _get_item_impl(
        ...
        masks = self._get_mask(datumaro_item, label, img_shape)
        img_info = ImageInfo(
            img_idx=index,
            img_shape=img_shape,
            ori_shape=img_shape,
    ...
    def _get_mask(self, datumaro_item: DatasetItem, label: np.ndarray, img_shape: tuple[int, int]) -> np.ndarray:
    ...
    def _bbox_to_mask(self, bbox: Bbox, img_shape: tuple[int, int]) -> np.ndarray:
        mask = np.zeros((1, *img_shape), dtype=np.uint8)
    ...
    def _get_label(self, datumaro_item: DatasetItem) -> np.ndarray:
        ...
        if isinstance(datumaro_item.media, ImageFromFile):
            ...
            # mean?
            label = AnomalyLabel.NORMAL if "good" in datumaro_item.id else AnomalyLabel.ANOMALOUS
        elif isinstance(datumaro_item.media, ImageFromBytes):
    ...
    def _map_id_to_label(self) -> dict[int, AnomalyLabel]:
    ...
    def _mask_image_from_file(self, datumaro_item: DatasetItem, img_shape: tuple[int, int]) -> np.ndarray:
        ...
        return np.zeros((1, *img_shape), dtype=np.uint8)
