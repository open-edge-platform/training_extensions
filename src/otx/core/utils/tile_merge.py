...
from otx.data.torch import TorchDataBatch, TorchPredBatch, TorchPredItem, TorchDataItem
...
class InstanceSegTileMerge(TileMerge):
    ...
    def merge(
        ...
        for tile_preds, tile_attrs in zip(batch_tile_preds, batch_tile_attrs, strict=True):
            ...
            for tile_attr, tile_img_info, tile_bboxes, tile_labels, tile_scores, tile_masks, tile_f_vect in zip(
                ...
                inst_seg_pred_entity = TorchPredItem(
                    image=torch.empty(tile_img_info.ori_shape),
                    img_info=tile_img_info,
                    bboxes=_bboxes,
                    labels=_labels,
    ...
    def _merge_entities(
        ...
        inst_seg_pred_entity = TorchPredItem(
            ...
            img_info=img_info,
            scores=scores,
            boxes=tv_tensors.BoundingBoxes(bboxes, canvas_size=img_size, format="XYXY"),
            label=labels,
            mask=tv_tensors.Mask(masks, dtype=bool),
        )
