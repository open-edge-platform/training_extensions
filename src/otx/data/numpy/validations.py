...
class ValidateItemMixin:
    ...
    @staticmethod
    def _feature_vector_validator(feature_vector: np.ndarray | np.ndarray) -> np.ndarray | np.ndarray:
        """Validate the feature vector.

        Numpy is mixed for this round as it is used in OV Classification.
        """
        if not isinstance(feature_vector, (np.ndarray, np.ndarray)):
            msg = "Feature vector must be a torch tensor or numpy array"
            raise TypeError(msg)
        if isinstance(feature_vector, np.ndarray) and feature_vector.dtype != torch.float32:
    ...
    def _saliency_map_validator(saliency_map: np.ndarray) -> np.ndarray:
        ...
        if not isinstance(saliency_map, np.ndarray):
            msg = "Saliency map must a numpy ndarray"
            raise TypeError(msg)
    ...
    def _boxes_validator(boxes: np.ndarray) -> np.ndarray:
        ...
        if not isinstance(boxes, np.ndarray):
            msg = "Boxes must be a numpy ndarray"
            raise TypeError(msg)
        return boxes
...
class ValidateBatchMixin:
    ...
    @staticmethod
    def _images_validator(image_batch: np.ndarray) -> np.ndarray:
        """Validate the image batch."""
        if not isinstance(image_batch, list) and not isinstance(image_batch, np.ndarray):
        ...
        if isinstance(image_batch, np.ndarray):
            ...
            if not all(isinstance(image, np.ndarray) for image in image_batch):
    ...
    def _labels_validator(label_batch: list[np.ndarray]) -> list[np.ndarray]:
        ...
        if all(label is None for label in label_batch):
            return []
        if not isinstance(label_batch, list) or not isinstance(label_batch[0], np.ndarray):
    ...
    @staticmethod
    def _scores_validator(scores_batch: list[np.ndarray | None]) -> list[np.ndarray]:
        """Validate the scores batch."""
        if all(score is None for score in scores_batch):
            return []
        if not isinstance(scores_batch, list) or not isinstance(scores_batch[0], np.ndarray):
    ...
    def _feature_vectors_validator(
        feature_vector_batch: list[np.ndarray | np.ndarray],
        ...
        # assumes homogeneous data so validation is done only for the first element
        # TODO(ashwinvaidya17): use only one dtype. Kept for OV Classification compatibility
        if isinstance(feature_vector_batch[0], np.ndarray) and not feature_vector_batch[0].dtype.is_floating_point:
        ...
        if isinstance(feature_vector_batch[0], np.ndarray) and feature_vector_batch[0].dtype.kind != "f":
            msg = f"Feature vector must have a floating point dtype. Got {feature_vector_batch[0].dtype}"
            raise ValueError(msg)
        if isinstance(feature_vector_batch[0], np.ndarray) and feature_vector_batch[0].ndim != 2:
            msg = "Feature vector must have 2 dimensions"
            raise ValueError(msg)
    ...
    def _saliency_maps_validator(
        saliency_map_batch: list[np.ndarray | np.ndarray | None],
    ) -> list[np.ndarray | np.ndarray]:
        ...
        if not isinstance(saliency_map_batch, list) or not isinstance(
            ...
            raise TypeError(msg)
        # assumes homogeneous data so validation is done only for the first element
        if isinstance(saliency_map_batch[0], np.ndarray) and not saliency_map_batch[0].dtype.is_floating_point:
            msg = f"Saliency map must have a floating point dtype. Got {saliency_map_batch[0].dtype}"
            raise ValueError(msg)
        return saliency_map_batch

    @staticmethod
    def _masks_validator(masks_batch: list[np.ndarray | None]) -> list[np.ndarray]:
    ...
    def _boxes_validator(boxes_batch: list[BoundingBoxes | None]) -> list[BoundingBoxes]:
        ...
        if all(box is None for box in boxes_batch):
            return []
        if not isinstance(boxes_batch, list) or not isinstance(boxes_batch[0], np.ndarray):
    ...
    @staticmethod
    def _keypoints_validator(keypoints_batch: list[np.ndarray | None]) -> list[np.ndarray]:
        """Validate the keypoints batch."""
        if all(keypoints is None for keypoints in keypoints_batch):
            return []
        if not isinstance(keypoints_batch, list) or not isinstance(keypoints_batch[0], np.ndarray):
