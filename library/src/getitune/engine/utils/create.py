...
def _read_backend(recipe_path: Path) -> str:
    ...
    Returns:
        Backend name string (e.g. ``'lightning'``, ``'openvino'``).
    """
...
def create_engine(
    ...
    * **Model instance** (``LightningModel``, ``OVModel``) or a **weights
      path** (``.xml``, ``.onnx``) — for OpenVINO and ONNX models.
    ...
    * **Model name** — a bare string (e.g. ``"yolox_s"``) that is resolved to a
    ...
    Args:
        ...
        **kwargs: Additional backend-specific keyword arguments
            (e.g., Trainer parameters for Lightning).
    ...
    for engine_cls in supported_engines:
        ...
        if engine_cls.is_supported(model, data):
            return engine_cls(model=model, data=data, **common_kwargs)  # type: ignore[call-arg]
