...
"""Module for Geti Tune custom f1 metrices."""
...
from getitune.metrics.mean_ap import MaskRLEMeanAveragePrecision
...
class FMeasure(Metric):
    ...
    def best_confidence_threshold(self) -> float:
        ...
        if isinstance(self._best_confidence_threshold, np.float64):
...
class MeanAveragePrecisionFMeasure(MetricCollection):
    ...
    NOTE: IMPORTANT!!! Do not use this metric to evaluate a F1 score on a test set.
        ...
        You should use `--metric getitune.core.metrics.fmeasure.FMeasureCallable`override
