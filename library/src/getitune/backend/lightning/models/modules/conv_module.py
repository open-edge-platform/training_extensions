...
class ConvModule(nn.Module):
    ...
    Args:
        ...
        bias (bool | str): If specified as `auto`, it will be decided by the normalization.
            ...
            Defaults to "auto".
        normalization (tuple[str, nn.Module] | nn.Module | None): Tuple of normalization layer name and module
            generated by ``build_norm_layer`` or instanstiated module.
            For the instanstiated module, the appropriate name will be automatically set.
            Defaults to None.
    ...
    def __init__(
        ...
        groups: int = 1,
        bias: bool | str = "auto",
        normalization: tuple[str, nn.Module] | nn.Module | None = None,
    ...
    def with_activation(self) -> bool:
        ...
        if self._with_activation is not None:
            # src/getitune/algo/segmentation/heads/fcn_head.py L144
    ...
    def with_activation(self, value: bool) -> None:
        ...
        For src/getitune/algo/segmentation/heads/fcn_head.py L144.
        """
...
def _patch_grad(x: Tensor) -> Tensor:
    """Patch the output gradient of Conv2d to workaround a known bug in PyTorch for XPU."""

    def hook(grad_x: Tensor) -> Tensor:
        """Hook to fix the output gradient of Conv2d."""
        stride = grad_x.stride()
        size = grad_x.size()

        # Calculate expected stride assuming that the tensor is contiguous.
        calc_stride = [*reversed(list(itertools.accumulate(reversed(size[1:]), operator.mul))), 1]
...
class PatchedConv2d(nn.Conv2d):
    """Patched Conv2d to fix the output gradient for XPU."""
