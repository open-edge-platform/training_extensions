...
from otx.backend.native.models.common.layers.transformer_layers import TransformerEncoder, TransformerEncoderLayer, get_activation
...
class CSPLayerv2(nn.Module):
    def __init__(self,
                 ...
                 act="silu",
                 bottletype=VGGBlock):
        super(CSPLayerv2, self).__init__()
...
class RepNCSPELAN4v2(nn.Module):
    # csp-elan
    def __init__(self, c1, c2, c3, c4, n=3,
        ...
        self.cv2 = nn.Sequential(CSPLayerv2(c3//2, c4, n, 1, bias=bias, act=act, bottletype=VGGBlock), ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act))
        self.cv3 = nn.Sequential(CSPLayerv2(c4, c4, n, 1, bias=bias, act=act, bottletype=VGGBlock), ConvNormLayer_fuse(c4, c4, 3, 1, bias=bias, act=act))
...
class DEIMHybridEncoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 dropout=0.0,
                 enc_act: Callable[..., nn.Module] = nn.GELU,
                 use_encoder_idx=[2],
        ...
        SCDown_Conv = nn.Sequential(SCDownV2(hidden_dim, hidden_dim, 3, 2))
...
class HybridEncoder:
    ...
    def __new__(cls, model_name: str) -> HybridEncoderModule:
        ...
        if model_name not in cls.encoder_cfg:
            ...
            raise KeyError(msg)
        if "deimv2" in model_name:
            return DEIMHybridEncoder(**cls.encoder_cfg[model_name])
