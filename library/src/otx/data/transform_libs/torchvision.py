...
class CachedMosaic(tvt_v2.Transform, NumpytoTVTensorMixin):
    ...
    def _create_mosaic_canvas(self, dtype: torch.dtype, device: torch.device) -> torch.Tensor:
        ...
        if isinstance(self.pad_val, (int, float)):
            fill_val = self.pad_val
        else:
            fill_val = self.pad_val[0]  # Use first channel value

        canvas = torch.full((3, mosaic_h, mosaic_w), fill_val, dtype=dtype, device=device)
        return canvas
    ...
    def _debug_visualize(self, sample, center_x: int, center_y: int) -> None:
        """Debug visualization - save annotated PNG for inspection."""
        import os
        ...
        os.makedirs("debug_mosaic", exist_ok=True)
...
class CachedMixUp(tvt_v2.Transform):
    ...
    def _translate_mask(self, mask: torch.Tensor, target_h: int, target_w: int, offset_x: int, offset_y: int) -> torch.Tensor:
    ...
    def _debug_visualize(
        ...
        pil_img.save(os.path.join("debug_images", filename))
    ...
    def forward(self, *_inputs: OTXSample) -> OTXSample:
        ...
        if isinstance(ori_img, np.ndarray):
            msg = f"Input image should be torch.Tensor, got numpy.ndarray. {msg}"
            raise TypeError(msg)
...
class TopdownAffine(tvt_v2.Transform, NumpytoTVTensorMixin):
    ...
    def __call__(self, *_inputs: OTXSample) -> OTXSample | None:
        ...
        if inputs.keypoints is None:
            ...
            inputs.keypoints[:, 2] = (
                inputs.keypoints[:, 2]
                * (inputs.keypoints[:, :2] >= 0).all(axis=1)
                * (inputs.keypoints[:, 0] <= w)
                * (inputs.keypoints[:, 1] <= h)
            )
