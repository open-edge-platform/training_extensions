...
class CachedCopyBlend(tvt_v2.Transform, NumpytoTVTensorMixin):
    """Implementation of CopyBlend augmentation with cache bank pattern.

    CopyBlend copies object patches from cached images and blends them into the current image.
    This helps with data augmentation by increasing object diversity and addressing class imbalance.

    Args:
        cache_max_size (int): Maximum number of images to keep in cache. Defaults to 20.
        num_objects (int): Number of objects to blend per image. Defaults to 3.
        area_threshold (float): Minimum area threshold for valid objects. Defaults to 0.0001.
        blend_ratio (float): Blending ratio (0=copy, 1=original). Defaults to 0.5.
        expand_ratios (tuple[float, float]): Range for random expansion of copied regions.
            Defaults to (1.0, 1.5).
        copyblend_type (str): Type of blending - 'blend' or 'copy'. Defaults to 'blend'.
        random_num_objects (bool): Whether to randomly vary number of objects. Defaults to True.
        with_expand (bool): Whether to expand the copied region. Defaults to True.
        bbox_clip_border (bool): Whether to clip bboxes at image border. Defaults to True.
        random_pop (bool): Whether to randomly pop from cache when full. Defaults to True.
        probability (float): Probability of applying this transformation. Defaults to 1.0.
        is_numpy_to_tvtensor (bool): Whether convert outputs to tensor. Defaults to True.
    """
    ...
    def __init__(
        ...
        expand_ratios: tuple[float, float] = (1.0, 1.5),
        copyblend_type: str = "blend",
        random_num_objects: bool = True,
        ...
        assert cache_max_size >= 2, f"Cache size must >= 2, but got {cache_max_size}."  # noqa: S101
        assert 0 <= probability <= 1.0, f"Probability should be in range [0,1], got {probability}."  # noqa: S101
        assert 0 <= blend_ratio <= 1.0, f"Blend ratio should be in range [0,1], got {blend_ratio}."  # noqa: S101
        assert copyblend_type in ("blend", "copy"), f"Invalid copyblend_type: {copyblend_type}"  # noqa: S101
    ...
    @cache_randomness
    def _get_valid_objects_indexes(self, cache: list[OTXDataItem]) -> list[int]:
        """Get indexes of cache items with valid objects.

        Args:
            cache: List of cached OTXDataItem objects.

        Returns:
            List of valid indexes where objects meet area threshold.
        """
        valid_indexes = []
        for idx, item in enumerate(cache):
            if len(item.bboxes) == 0:
                continue
            # Calculate areas from bboxes (XYXY format)
            bboxes = item.bboxes
            areas = (bboxes[:, 2] - bboxes[:, 0]) * (bboxes[:, 3] - bboxes[:, 1])
            # Normalize by image size
            img_h, img_w = item.img_info.img_shape
            normalized_areas = areas / (img_h * img_w)
            # Check if any object meets threshold
            if torch.any(normalized_areas >= self.area_threshold):
                valid_indexes.append(idx)
        return valid_indexes

    @cache_randomness
    def _select_objects_from_cache(
        self,
        cache_item: OTXDataItem,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Select valid objects from a cached item.

        Args:
            cache_item: Cached OTXDataItem to extract objects from.

        Returns:
            Tuple of (bboxes, labels, areas) for valid objects.
        """
        bboxes = cache_item.bboxes  # XYXY format
        labels = cache_item.label

        # Calculate normalized areas
        img_h, img_w = cache_item.img_info.img_shape
        areas = (bboxes[:, 2] - bboxes[:, 0]) * (bboxes[:, 3] - bboxes[:, 1])
        normalized_areas = areas / (img_h * img_w)

        # Filter by area threshold
        valid_mask = normalized_areas >= self.area_threshold
        ...
        num_to_select = self.num_objects
        if self.random_num_objects:
            num_to_select = RNG.integers(1, min(self.num_objects, len(valid_bboxes)) + 1)
        else:
            num_to_select = min(self.num_objects, len(valid_bboxes))

        selected_idx = RNG.choice(len(valid_bboxes), size=num_to_select, replace=False)

        return valid_bboxes[selected_idx], valid_labels[selected_idx]

    @typing.no_type_check
    def forward(self, *_inputs: OTXDataItem) -> OTXDataItem | None:
        """Apply CopyBlend augmentation using cache bank.

        Args:
            _inputs: Input OTXDataItem.

        Returns:
            Augmented OTXDataItem or None.
        """
        assert len(_inputs) == 1, "[tmp] Multiple entity is not supported yet."  # noqa: S101
        inputs = _inputs[0]

        # Add current input to cache
        self.results_cache.append(copy.deepcopy(inputs))
        if len(self.results_cache) > self.cache_max_size:
            index = RNG.integers(0, len(self.results_cache) - 1) if self.random_pop else 0
            self.results_cache.pop(index)

        # Need at least 2 images in cache to perform augmentation
        if len(self.results_cache) <= 1:
            return self.convert(inputs)
        ...
        # Get valid cache items with objects
        valid_indexes = self._get_valid_objects_indexes(self.results_cache)
        ...
        # Create working copy
        result_img = ori_img.astype(np.float32)
        blended_bboxes = []
        blended_labels = []

        # Select source from cache
        source_idx = RNG.choice(valid_indexes)
        source_item = self.results_cache[source_idx]
        source_img: np.ndarray = to_np_image(source_item.image)
        source_h, source_w = source_img.shape[:2]

        # Get objects to blend
        obj_bboxes, obj_labels = self._select_objects_from_cache(source_item)
        ...
        for bbox, label in zip(obj_bboxes, obj_labels):
            # Convert XYXY to pixel coordinates in source image
            x1_src = int(bbox[0].item())
            y1_src = int(bbox[1].item())
            x2_src = int(bbox[2].item())
            y2_src = int(bbox[3].item())

            # Clip to source image bounds
            x1_src = max(0, min(x1_src, source_w - 1))
            y1_src = max(0, min(y1_src, source_h - 1))
            x2_src = max(0, min(x2_src, source_w))
            y2_src = max(0, min(y2_src, source_h))
            ...
            # Handle expansion
            if self.with_expand:
                alpha = RNG.uniform(self.expand_ratios[0], self.expand_ratios[1])
                expand_w = int(obj_w * (alpha - 1.0) / 2)
                expand_h = int(obj_h * (alpha - 1.0) / 2)

                # Expand in source image
                x1_src = max(0, x1_src - expand_w)
                y1_src = max(0, y1_src - expand_h)
                x2_src = min(source_w, x2_src + expand_w)
                y2_src = min(source_h, y2_src + expand_h)

                obj_w = x2_src - x1_src
                obj_h = y2_src - y1_src

            # Extract patch from source
            source_patch = source_img[y1_src:y2_src, x1_src:x2_src]

            # Randomly determine position in target image
            if obj_w < img_w and obj_h < img_h:
                x1_dst = RNG.integers(0, img_w - obj_w)
                y1_dst = RNG.integers(0, img_h - obj_h)
            else:
                # Object too large, skip
                continue
            ...
            if self.copyblend_type == "blend":
                result_img[y1_dst:y2_dst, x1_dst:x2_dst] = (
                    self.blend_ratio * result_img[y1_dst:y2_dst, x1_dst:x2_dst]
                    + (1.0 - self.blend_ratio) * source_patch.astype(np.float32)
                )
            else:  # copy
                result_img[y1_dst:y2_dst, x1_dst:x2_dst] = source_patch.astype(np.float32)

            # Create new bbox in XYXY format
            new_bbox = torch.tensor([x1_dst, y1_dst, x2_dst, y2_dst], dtype=torch.float32)
            blended_bboxes.append(new_bbox)
            blended_labels.append(label)

        # Update inputs
        inputs.image = result_img.astype(np.uint8)

        # Combine original and blended bboxes/labels
        if len(blended_bboxes) > 0:
            blended_bboxes = torch.stack(blended_bboxes)
            blended_labels = torch.stack(blended_labels)

            combined_bboxes = torch.cat([inputs.bboxes, blended_bboxes], dim=0)
            combined_labels = torch.cat([inputs.label, blended_labels], dim=0)

            # Clip bboxes if needed
            if self.bbox_clip_border:
                combined_bboxes = clip_bboxes(combined_bboxes, (img_h, img_w))
    ...
    def visualize(
        ...
        for idx, bbox in enumerate(bboxes):
            ...
            color = (0, 255, 0) if idx < len(bboxes) - self.num_objects else (255, 0, 255)
    ...
    def __repr__(self) -> str:
        repr_str = self.__class__.__name__
        repr_str += f"(cache_max_size={self.cache_max_size}, "
        repr_str += f"num_objects={self.num_objects}, "
        repr_str += f"area_threshold={self.area_threshold}, "
        repr_str += f"blend_ratio={self.blend_ratio}, "
        repr_str += f"expand_ratios={self.expand_ratios}, "
        repr_str += f"copyblend_type={self.copyblend_type}, "
        repr_str += f"random_num_objects={self.random_num_objects}, "
        repr_str += f"with_expand={self.with_expand}, "
        repr_str += f"bbox_clip_border={self.bbox_clip_border}, "
        repr_str += f"random_pop={self.random_pop}, "
        repr_str += f"prob={self.prob}, "
        repr_str += f"is_numpy_to_tvtensor={self.is_numpy_to_tvtensor})"
