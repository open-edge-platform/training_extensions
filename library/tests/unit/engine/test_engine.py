...
class TestCreateEngine:
    ...
    def test_create_engine(self, mock___subclasses__, mock_ov_is_supported, mock_lt_is_supported, mock_engine_subclass):
        ...
        with pytest.raises(ValueError, match="No engine found for model .* and data .*"):
            create_engine(mock_model, mock_data)  # type: ignore[arg-type]
...
class TestResolveRecipe:
    ...
    def test_model_name_unique(self):
        ...
        result = _resolve_recipe("yolox_s", task=None)
        assert result.name == "yolox_s.yaml"
...
class TestReadBackend:
    ...
    def test_read_backend_explicit(self, tmp_path):
        """Recipe with explicit backend field returns that backend."""
        recipe = tmp_path / "recipe.yaml"
        recipe.write_text("backend: openvino\n")
        assert _read_backend(recipe) == "openvino"
