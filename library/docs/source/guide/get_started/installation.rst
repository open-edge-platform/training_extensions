...
The current version of Geti Library was tested in the following environment:
...
The easiest way to install Geti Library is via PyPI (`getitune <https://pypi.org/project/getitune>`_ package).
...
If you want to install Geti Library from source, you have to first clone the repository,
...
*****************************************************
Install Geti Library by using Docker
*****************************************************

1. By executing the following commands, it will build two
Docker images: ``getitune:${VERSION}-cuda`` and ``getitune:${VERSION}-cuda-pretrained-ready``.
...
.. code-block:: shell
    ...
    getitune                                           2.0.0-cuda-pretrained-ready   4f3b5f98f97c   3 minutes ago   14.5GB
    getitune                                           2.0.0-cuda                    8d14caccb29a   8 minutes ago   10.4GB

``getitune:${VERSION}-cuda`` is a minimal Docker image with CUDA support.
``getitune:${VERSION}-cuda-pretrained-ready`` includes pre-trained models on top of the base image.
